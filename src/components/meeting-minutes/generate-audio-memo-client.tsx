...
interface GenerateAudioMemoClientProps {
  contracts: Contract[];
  currentUser?: User;
}

const GenerateAudioMemoClient: FC<GenerateAudioMemoClientProps> = ({ contracts, currentUser }) => {
  ...
  const handleGenerateAndSave = async () => {
    if (!audioFile || !selectedContractId) {
      toast({ title: "Dados Incompletos", description: "Por favor, selecione um contrato e um arquivo de áudio.", variant: "destructive" });
      return;
    }
    ...
    try {
      ...
      reader.onload = async () => {
        ...
        if (result.success && result.data) {
          toast({
            title: "Ata de Reunião Gerada e Salva!",
            description: "Você será redirecionado para a tela de edição para revisar e adicionar anexos.",
          });
          
          if (!result.data.id || result.data.id === 'undefined') {
            console.error('ID da ata inválido após geração:', result.data.id);
            toast({
              title: "Erro de Navegação",
              description: "ID da ata inválido. Não é possível redirecionar.",
              variant: "destructive",
            });
            return;
          }
          ...
          toast({
            title: "Falha na Geração da Ata",
            description: result.message || "Não foi possível processar o áudio.",
            variant: "destructive",
          });
           setIsLoading(false);
        }
      ...
      toast({ title: "Falha no Upload", description: "Não foi possível carregar o arquivo. Por favor, tente novamente.", variant: "destructive" });
      setIsLoading(false);
    }
  };
...
export default GenerateAudioMemoClient;
